...
Require-File 'frontend-ui/Dockerfile'
...
if ($LASTEXITCODE -ne 0) { throw 'Docker Compose is not available. Install/update Docker Desktop and retry.' }
...
& docker @args
if ($LASTEXITCODE -ne 0) { throw 'Docker Compose failed to start the application.' }
...
while ((Get-Date) -lt $deadline) {
    ...
    $backendState = if ($backendId) { (docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}' $backendId 2>$null).Trim() } else { '' }
    $frontendState = if ($frontendId) { (docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}' $frontendId 2>$null).Trim() } else { '' }
...
if (-not $frontendPort) { throw 'Frontend started but its host port could not be discovered.' }

$frontendUrl = "http://$frontendPort"
$backendUrl = if ($backendPort) { "http://$backendPort" } else { '(internal / not published)' }
